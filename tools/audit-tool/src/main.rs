...
async fn index() -> Html<String> {
    let html = concat!(
        ...
        ".middle { flex: 3; display: flex; gap: 20px; padding: 10px; overflow: hidden; }",
        ".bottom { flex: 1; border-top: 1px solid var(--pico-secondary-border-color); padding: 10px 20px; overflow: auto; }",
        ...
        ".view-toggle button { padding: 4px 8px; font-size: 0.8rem; margin-right: 5px; }",
        ".view-toggle button.active { background: var(--pico-primary-background); }",
        ...
        "<button class=\"active\" onclick=\"setView('pretty')\">Pretty</button>",
        "<button onclick=\"setView('raw')\">Raw JSON</button>",
        "</div>",
