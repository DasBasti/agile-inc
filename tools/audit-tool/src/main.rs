...
async fn index() -> Html<String> {
    let html = concat!(
        ...
        "<style>",
        ":root { --pico-font-size: 14px; --pico-background: #ffffff; }",
        "body { display: flex; flex-direction: column; height: 100vh; margin: 0; padding: 0; }",
        "main { flex: 1; display: flex; flex-direction: column; padding: 10px; max-width: 100%; }",
        ".header { display: flex; justify-content: space-between; align-items: center; padding: 10px 20px; border-bottom: 1px solid var(--pico-secondary-border-color); }",
        ".header h1 { margin: 0; font-size: 1.5rem; }",
        ".header .status { display: flex; gap: 20px; font-size: 0.9rem; }",
        ".middle { flex: 3; display: flex; gap: 20px; padding: 10px; overflow: auto; min-height: 0; }",
        ".bottom { flex: 1; min-height: 150px; border-top: 1px solid var(--pico-secondary-border-color); padding: 10px 20px; overflow: auto; }",
        ".left-panel, .right-panel { flex: 1; display: flex; flex-direction: column; overflow: auto; min-width: 0; }",
        ".left-panel h2, .right-panel h2, .bottom h2 { margin: 0 0 10px 0; font-size: 1.1rem; }",
        ".doc-list { flex: 1; overflow: auto; border: 1px solid var(--pico-secondary-border-color); }",
        ".doc-list table { margin: 0; font-size: 0.85rem; }",
        ".doc-list th, .doc-list td { padding: 6px 8px; }",
        ".doc-detail { flex: 1; overflow: auto; border: 1px solid var(--pico-secondary-border-color); padding: 10px; background: var(--pico-secondary-background); }",
        ".doc-detail pre { margin: 0; white-space: pre-wrap; word-break: break-all; font-size: 0.85rem; }",
        ".mqtt-msg { padding: 6px; margin: 3px 0; background: var(--pico-secondary-background); border-radius: 4px; font-family: monospace; font-size: 0.8rem; }",
        ".mqtt-msg .topic { color: var(--pico-primary-color); font-weight: bold; }",
        ".status-success { color: #28a745; }",
        ".status-error { color: #dc3545; }",
        ".tag { background: var(--pico-primary-background); color: var(--pico-primary-color); padding: 2px 6px; border-radius: 4px; font-size: 0.75em; }",
        ".view-toggle { margin-bottom: 10px; }",
        ".view-toggle button { padding: 4px 8px; font-size: 0.8rem; margin-right: 5px; background: var(--pico-secondary-background); border: 1px solid var(--pico-secondary-border-color); }",
        ".view-toggle button.active { background: var(--pico-primary-background); color: var(--pico-primary-color); }",
        ".field-group { margin-bottom: 15px; }",
        ".field-group h3 { font-size: 1rem; margin-bottom: 8px; color: var(--pico-primary-color); }",
        ".field-group p { margin: 4px 0; font-size: 0.9rem; }",
        ".field-label { font-weight: bold; color: var(--pico-muted-color); font-size: 0.8rem; }",
        "</style>",
        "</head>",
        "<body>",
        "<main>",
        ...
        "<button class=\"active\" onclick=\"setView('pretty', this)\">Pretty</button>",
        "<button onclick=\"setView('raw', this)\">Raw JSON</button>",
        "</div>",
        ...
        "  if (detail.dataset.docId) {",
        "    htmx.ajax('GET', '/api/docs/' + detail.dataset.docId + '?view=' + view, {target: '#doc-detail'});",
        "  }",
        "}",
